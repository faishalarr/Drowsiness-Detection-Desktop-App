...
def run_detect():
    ...
    def detection1(img, net, classes): 
        ...
        for i in indices:
            ...
            cv2.moveWindow("Detected Faces", 670, 670);
        ...
        cv2.moveWindow("Detection Info", 830,0);
...
detect = customtkinter.CTkButton(master=root,width=120,
                                 ...
                                 border_width=0,
                                 corner_radius=8,
                                 text="DETECT REALTIME",  
                                 command=run_detect)
detect.pack(padx=20, pady=10)
detect.place(x=885, y=570, anchor="n")
...
def dataprep():
    ...
    def hapus():
        ...
        if result is True:
            messagebox.showinfo("Sukses", "Dataset berhasil dihapus",parent=model_dataprep)
        else:
            messagebox.showerror("Kesalahan", f"Terjadi kesalahan: {result}", parent=model_dataprep)
    

    def open_directory():
        global image_files
        image_files = [f for f in os.listdir(saved) if f.lower().endswith(('.jpg', '.jpeg', '.png', '.gif'))]
    ...
    def show_image(image_index):
        if image_files:
            image_path = os.path.join(saved, image_files[image_index])
    ...
    progress_label = customtkinter.CTkLabel(tabview.tab("DATASET"), text="Dataset anda akan diubah ukurannya menjadi ukuran 640 x 480 piksel", wraplength=300, font=("Arial", 20))
    ...
    hapus_button = customtkinter.CTkButton(tabview.tab("DATASET"), text="HAPUS DATASET", command=hapus)
    hapus_button.pack(pady=10)

    open_button = customtkinter.CTkButton(tabview.tab("DATASET"), text="TAMPILKAN HASIL", state=tk.DISABLED,command=open_directory)
    ...
    status_label.pack()
    status_label.place(relx=0.5, y=220, anchor="n")

    image_label = tk.Label(tabview.tab("DATASET"))
    image_label.pack()
    image_label.place(relx=0.5, y=350, anchor="n")
...
def run_eval(): 
    ...
    def select_weights_directory():
        ...
        subprocess.run(f'start cmd /c "{commandeval}"', shell=True)
    ...
    def show_image(image_patha,image_indexx):
        akurasi.configure(text="Accuracy: 84%   Precision: 85%   Recall: 83%")
        if image_filea:
    ...
    button_1 = customtkinter.CTkButton(tabview.tab("EVALUATION"), text="EVALUASI MODEL",state=tk.DISABLED,command=select_weights_directory)
    button_1.pack(padx=20, pady=20)

    open_button = customtkinter.CTkButton(tabview.tab("EVALUATION"), text="TAMPILKAN HASIL", state=tk.DISABLED,command=open_directory)
    open_button.pack(pady=10)

    akurasi = customtkinter.CTkLabel(tabview.tab("EVALUATION"), text="Accuracy:   Precision:   Recall: ", wraplength=300, font=("Arial", 18))
    akurasi.pack(padx=20, pady=10)

    prev_button = customtkinter.CTkButton(tabview.tab("EVALUATION"), text="Previous", state=tk.DISABLED, command=show_prev_image)
    prev_button.pack(side=tk.LEFT, padx=10)

    next_button = customtkinter.CTkButton(tabview.tab("EVALUATION"), text="Next", state=tk.DISABLED, command=show_next_image)
    next_button.pack(side=tk.RIGHT, padx=10)

    status_label = customtkinter.CTkLabel(tabview.tab("EVALUATION"), text="", pady=10)
    status_label.pack()

    image_label = tk.Label(tabview.tab("EVALUATION"))
    image_label.pack()
...
def run_modeling(): 
    ...
    tabview.add("PELATIHAN")
    tabview.add("CONVERT")
    tabview.set("PELATIHAN")
    ...
    def run_export():
        ...
        command = f"python export.py --weights runs/train/exp/weights/best.pt --include torchscript onnx"
    ...
    progress_split = customtkinter.CTkLabel(tabview.tab("PELATIHAN"), text="Dataset akan displit dengan ratio 90:10", wraplength=300, font=("Arial", 20))
    progress_split.pack(padx=20, pady=10)

    splitt = customtkinter.CTkButton(tabview.tab("PELATIHAN"), text="SPLIT DATASET", command=splitdata)
    splitt.pack(padx=20, pady=10)

    batch_label = customtkinter.CTkLabel(tabview.tab("PELATIHAN"), text="Batch Size:")
    batch_label.pack()
    batch_entry = Entry(tabview.tab("PELATIHAN"))
    batch_entry.pack()

    epoch_label = customtkinter.CTkLabel(tabview.tab("PELATIHAN"), text="Number of Epochs:")
    epoch_label.pack(padx=20, pady=10)
    epoch_entry = Entry(tabview.tab("PELATIHAN"))
    epoch_entry.pack(padx=20, pady=10)

    latihh = customtkinter.CTkButton(tabview.tab("PELATIHAN"), text="LATIH DATASET", state=tk.DISABLED,command=start_training)
    latihh.pack(padx=20, pady=10)

    hadeh = customtkinter.CTkLabel(tabview.tab("PELATIHAN"), text="Performa model saat pelatihan", wraplength=300, font=("Arial", 20))
    hadeh.pack(padx=20, pady=10)

    # Create the first treeview widget to display the original column data
    tree1 = ttk.Treeview(tabview.tab("PELATIHAN"), columns=('mAP 0.5',), show='headings')
    tree1.heading('#1', text='mAP 0.5')
    tree1.pack()
    tree1.place(x=885, y=480, anchor="n")

    # Create the second treeview widget to display the mean value
    tree2 = ttk.Treeview(tabview.tab("PELATIHAN"), columns=('Avg mAP 0.5',), show='headings')
    tree2.heading('#1', text='Avg mAP 0.5')
    tree2.pack()
    tree2.place(x=1050, y=480, anchor="n")

    show_button = customtkinter.CTkButton(tabview.tab("PELATIHAN"), text="Show", state=tk.DISABLED,command=show_column_data)
    show_button.pack()
    ...
    # next_button = customtkinter.CTkButton(tabview.tab("PELATIHAN"), text="Next", state=tk.DISABLED, command=show_next_image)
    # next_button.pack(side=tk.RIGHT, padx=10)
...
def run_detekinput(): 
    ...
    def select_directory():
        delete_directory("runs/detect/exp")  # Delete the directory

        directory_path = "data/images"
        delete_files(directory_path)  # Delete files in the directory
        
        file_paths = filedialog.askopenfilenames(parent=detekinput_window, filetypes=[("JPEG files", "*.jpg")], title="Select JPEG files")
        
        if file_paths:
            move_files(file_paths, directory_path)
        
        detekk.configure(state=tk.NORMAL)
    
    select_button = customtkinter.CTkButton(tabview.tab("INPUT GAMBAR"), text="INPUT GAMBAR", command=select_directory)
    select_button.pack(padx=20, pady=20)

    detekk = customtkinter.CTkButton(tabview.tab("INPUT GAMBAR"), text="Detect",state=tk.DISABLED, command=Detect)
    detekk.pack(padx=20, pady=10)

    open_button = customtkinter.CTkButton(tabview.tab("INPUT GAMBAR"), text="TAMPILKAN HASIL",state=tk.DISABLED, command=open_directory)
    open_button.pack(pady=10)

    prev_button = customtkinter.CTkButton(tabview.tab("INPUT GAMBAR"), text="Previous", state=tk.DISABLED, command=show_prev_image)
    prev_button.pack(side=tk.LEFT, padx=10)

    next_button = customtkinter.CTkButton(tabview.tab("INPUT GAMBAR"), text="Next", state=tk.DISABLED, command=show_next_image)
    next_button.pack(side=tk.RIGHT, padx=10)

    status_label = customtkinter.CTkLabel(tabview.tab("INPUT GAMBAR"), text="", pady=10)
    status_label.pack()

    image_label = tk.Label(tabview.tab("INPUT GAMBAR"))
    image_label.pack()
...
detect_i.place(x=685, y=570, anchor="n")
