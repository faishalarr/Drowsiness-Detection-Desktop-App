...
def dataprep():
    ...
    # Loop through all files in the directory
        for filename in os.listdir(directory):
            ...
            if filename.lower().endswith((".jpg", ".jpeg")):
                ...
                print(f"Resized {filename} to {new_filename} and deleted original")

    def select_and_resize():
        root = tk.Tk()
        root.withdraw()  # Hide the main window
        directory = filedialog.askdirectory(parent=model_dataprep,title="Select a directory")
        if directory:
            resize(directory)
            print("Resizing images in selected directory:", directory)
    ...
    def hasil():
        directory = filedialog.askdirectory(parent=model_dataprep,title="Pilih Direktori")
        if directory:
    ...
    button_2 = customtkinter.CTkButton(tabview.tab("HASIL"), text="HASIL",command=hasil)
    button_2.pack(padx=20, pady=20)
